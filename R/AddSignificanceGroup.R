...
#' @export
AddSignificanceGroup <- function(.data, .foldChangeVar, .pValueVar, .threshold=0.05) {
  ...
  .data %>%
    mutate(significanceGroup = case_when(
      (!!.pValueVar > -log10(.threshold) & !!.foldChangeVar < - 0 ) ~ 'Significant (down)',
      (!!.pValueVar > -log10(.threshold) & !!.foldChangeVar >=  0 ) ~ 'Significant (up)'
    )) %>%
