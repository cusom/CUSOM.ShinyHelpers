...
#' @param foldChangeVariable name of fold change column
...
#' @export
addSignificanceGroup <- function(.data, foldChangeVar, significanceVariable, adjustedInd, significanceThreshold, originalSignificanceThreshold) {
  ...
  .data %>% 
    mutate(
      significanceGroup = case_when(
        (!!significanceVariable > significanceThreshold & !!foldChangeVar < -0) ~ glue("down ({significanceLetter} < {originalSignificanceThreshold})"), 
        (!!significanceVariable > significanceThreshold & !!foldChangeVar >= 0) ~ glue("up ({significanceLetter} < {originalSignificanceThreshold})"), 
